...
class _AddProductPageState extends State<AddProductPage> {
    ...
    return Container(
      ...
      child: LayoutBuilder(
        builder: (BuildContext context, BoxConstraints constraints) {
          return (constraints.maxWidth > 800) ? _rowView(constraints) : _columnView(constraints);
        },
      ),
    );
  }
  ...
  Widget productForm(BoxConstraints constraints) {
    return SizedBox(
      ...
      width: (constraints.maxWidth > 800) ? constraints.maxWidth / 2 : constraints.maxWidth,
      child: Form(
        key: _formKey,
        child: ListView(
                                  ...
                                  setState(() {
                                    if (_options[index] == "") {
                                      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                                        content: Text("Ingresa el nombre del item"),
                                        duration: Duration(seconds: 2),
                                      ));
                                      return;
                                    }
                                    if (!_subproducts.containsKey(_options[index])) {
                            ...
                            return Row(
                              children: [
                                ...
                                Expanded(
                                  child: CustomInputText(
                                    valueCallback: (val) =>
                                        _subproducts[_options[index]][item] = val,
                                    label: "Opción ${item + 1}",
